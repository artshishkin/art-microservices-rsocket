...
@Slf4j
@DisplayName("Start server `Lec04PersistentConnectionServer`, run test, during 15s pause restart server - will catch java.nio.channels.ClosedChannelException")
class Lec04PersistentConnectionManualTest {
    ...
    void connectionTest() throws InterruptedException {
        ...
        //when
        Flux<String> flux1 = rSocket.requestStream(payload)
        ...
        Flux<String> flux2 = rSocket.requestStream(payload)
                .delayElements(Duration.ofMillis(50)) //slow client process emulation
                .map(Payload::getDataUtf8)
                .doOnNext(dto -> log.debug("client: receive {}", dto))
                .take(10);
